...
elseif(Args.AnalysisLevel)
	...
	r = 'Single';
elseif((Args.TrialType) && ~(Args.EventMarkers) && ~(Args.TimeInterval))
...
elseif(Args.EventMarkers && Args.TimeInterval && Args.TrialType)
    ind = [];
        if ~isempty(strmatch('Pancake',strsplit(pwd,'/'))) || ~isempty(strmatch('James',strsplit(pwd,'/')))
            tr = trialsOld('auto',varargin{:});
        else
        ...
        for j = 1:length(ind)
            if strcmpi(Args.EventMarkers,'target')
                if ~isempty(strmatch('Pancake',strsplit(pwd,'/'))) || ~isempty(strmatch('James',strsplit(pwd,'/')))
                    time_onset = tr.data.trials(i).target.timestamp - bins(1);
                    time_offset = tr.data.trials(i).target.timestamp + bins(2);
                else
                    time_onset = tr.data.trials(i).target.onset - bins(1);
                    time_offset = tr.data.trials(i).target.onset + bins(2);
                end
            ...
            elseif strcmpi(Args.EventMarkers,'distractor')
                if ~isempty(strmatch('Pancake',strsplit(pwd,'/'))) || ~isempty(strmatch('James',strsplit(pwd,'/')))
                    time_onset = tr.data.trials(i).distractors(1) - bins(1);
                    time_offset = tr.data.trials(i).distractors(1) + bins(2);
                else
                    time_onset = tr.data.trials(i).distractor.onset - bins(1);
                    time_offset = tr.data.trials(i).distractor.onset + bins(2);
                end
                tr_ind = find(obj.data.trialIdx==ind(j) & (obj.data.spiketimes>time_onset & obj.data.spiketimes < time_offset));
                sptimes = [sptimes; obj.data.spiketimes(tr_ind)];
                tIdx = [tIdx; obj.data.trialidx(tr_ind)];
            ...
            elseif strcmpi(Args.EventMarkers,'saccade')
                if ~isempty(strmatch('Pancake',strsplit(pwd,'/'))) || ~isempty(strmatch('James',strsplit(pwd,'/')))
                    time_onset = tr.data.trials(i).saccade.timestamp - bins(1);
                    time_offset = tr.data.trials(i).saccade.timestamp + bins(2);
                else
                    time_onset = tr.data.trials(i).saccade.onset - bins(1);
                    time_offset = tr.data.trials(i).saccade.onset + bins(2);
                end
                tr_ind = find(obj.data.trialIdx==ind(j) & (obj.data.spiketimes>time_onset & obj.data.spiketimes < time_offset));
                sptimes = [sptimes; obj.data.spiketimes(tr_ind)];
                tIdx = [tIdx; obj.data.trialidx(tr_ind)];
